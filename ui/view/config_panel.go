...
func (v *configPanel) Build(startRow int, parent ...Widget) (row int) {
	...
	makeRow("useColorDetect", "Color Detect (true/false)", fmt.Sprintf("%t", c.UseColorDetect))
...
func (v *configPanel) ApplyChanges() {
	...
	assignInt("roiSizePx", &cfg.ROISizePx)
	...
	assignBool("useColorDetect", &cfg.UseColorDetect)
	assignInt("colorRedDelta", &cfg.ColorRedDelta)
